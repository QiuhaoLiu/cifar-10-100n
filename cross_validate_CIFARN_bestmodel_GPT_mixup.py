...
def main(args):
    ...
    output_file = os.path.join(
        ...
        f"Best_model_Mixup_{args.dataset}_trainset_pyx_{args.noise_type}_{args.k_folds}fold_{args.n_epoch}epoch.npy"
    )
    output_file_pred_label = os.path.join(
        args.output_dir_label,
        f"Best_model_Mixup_{args.dataset}_trainset_pyx_argmax_predicted_labels_{args.noise_type}_{args.k_folds}fold_{args.n_epoch}epoch.npy"
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(num_training_samples))):
        ...
        for epoch in range(args.n_epoch):
            ...
            for images, labels, _ in tqdm(train_loader, desc=f"Epoch = {epoch + 1}", leave=False):
                ...
                #应用Mixup
                mixed_images, targets_a, targets_b, lam = mixup_data(images, labels, alpha=1.0, use_cuda=True)
